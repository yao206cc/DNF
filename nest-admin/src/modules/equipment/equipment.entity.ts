...
export class EquipmentEntity extends CommonEntity {
   ...
   * 装备适用等级。
   */
  @Column({ name: 'equipment_level', type: 'int', nullable: false })
  applicableLevel: number

  /**
   * 适用职业。
   */
  @Column({ name: 'applicable_class', type: 'varchar', length: 255, nullable: false })
  applicableClass: string

  /**
   * 装备品质。
   */
  @Column({ name: 'equipment_quality', type: 'varchar', length: 255, nullable: false })
  equipmentQuality: string

  /**
   * 穿戴部位。
   */
  @Column({ name: 'wear_part', type: 'varchar', length: 255, nullable: false })
  wearPart: string

  /**
   * 武器类型。
   */
  @Column({ name: 'weapon_type', type: 'varchar', length: 255, nullable: false })
  weaponType: string
   ...
   * 生命值。
   */
  @Column({ name: 'healthPoints', type: 'int', nullable: true })
  healthPoints: number
